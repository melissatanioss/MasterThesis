...
AVAILABLE_CONSTR_TAG = [
    Tags.PROPERTY, Tags.TCC, Tags.CCC
]
...
class ProcessNode:
    def __init__(self, xml_element: ET.Element) -> None:
        ...
        self.default = parse_default_branch(attribs.get(Attributes.DEFAULT)) if attribs.get(Attributes.DEFAULT) != None else None
...
def parse(path):
    ...
    users_list = check_constraints_ccc(root[1])
    ...
    (properties, constraints, counter_constraints) = check_constraints(root[1], id_list, users_list)
    
    return (process, properties, constraints, counter_constraints)
...
def check_constraints_ccc(process: ET.Element):
    """
        Check if the process block is correctly defined
    """
    # Check tag
    if process.tag != Tags.CONSTR:
        sys.exit(f"SYNTAX ERROR: Invalid tag '{process.tag}'. '{Tags.CONSTR}' tag is required")
    users_list = {}

    for element in process:
        if element.tag == Tags.CCC:
            users_list[element.text.split()[0]] = process.tag
    return users_list
...
def parse_resources(res_string):
    ...
    for r in resources:
        ...
        if match == None:
            sys.exit(f"SYNTAX ERROR: '{r}' element is not well formed. It may contain only lowercase/uppercase letters and numbers, separated by an underscore")
...
def check_xor(xor_elem: ET.Element, id_list):
    ...
    if default != None:
        ...
        if xor_elem.get(Attributes.CONTROLLABLE_DEFAULT) == None:
            sys.exit(f"SYNTAX ERROR: '{Attributes.CONTROLLABLE_DEFAULT}' is required since '{Attributes.DEFAULT}' has been specified")
...
def parse_default_branch(default_branch: str):
    ...
    if match == None:
        sys.exit(f"SYNTAX ERROR: '{default_branch}' is not well formed. It may contain only lowercase/uppercase letters and numbers, separated by an underscore")
...
def get_id(element: ET.Element):
    ...
    if id == None:
        sys.exit(f"SYNTAX ERROR: '{Attributes.ID}' attribute not present for tag '{element.tag}'. '{Attributes.ID}' attribute is required")

    match = re.search(BLOCK_ID_REGEXP, id)
    if match == None:
        sys.exit(f"SYNTAX ERROR: '{id}' id is not well formed. It may contain only lowercase/uppercase letters and numbers, separated by an underscore")
...
def parse_guard(guard: str):
    ...
    if match == None:
        sys.exit(f"SYNTAX ERROR: '{guard}'  is not well formed. It may contain only lowercase/uppercase letters and numbers, separated by an underscore")

    return guard

def check_constraints(constr: ET.Element, task_ids: dict, users_ids: dict):
    """
    ...
    users_ids_list = list(users_ids.keys())
    for element in constr:
        if element.tag == Tags.PROPERTY:
            ...
            tcc = parser_constraints.parse(element.text, task_ids_list, list(properties.keys()), True)
            constraints.append(tcc)
        elif element.tag == Tags.CCC:
            ccc = parser_constraints.parse(element.text, users_ids_list, ["<", "<="], False)
            counter_constraints.append(ccc)
        else:
            sys.exit(f"SYNTAX ERROR: Invalid tag '{element.tag}'.")

    return (properties, constraints, counter_constraints)
